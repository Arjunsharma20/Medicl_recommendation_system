...
model_path = os.path.join(BASE_DIR, "health_predictor.pkl")
...
# Your symptoms_dict and diseases_list go here (same as before)
symptoms_dict = {
    ...
    'internal_itching': 93, 'toxic_look_typhos': 94, 'depression': 95, 'irritability': 96,
    ...
    'fluid_overload_1': 117, 'blood_in_sputum': 118, 'prominent_veins_on_calf': 119,
...
diseases_list = {
    ...
    32: 'Paralysis (brain hemorrhage)', 33: 'Peptic ulcer disease', 34: 'Pneumonia',
...
def helper(dis):
    ...
    desc = " ".join(description[description['Disease'].str.lower() == dis]['Description'].values) \
           or "No description available."

    pre = precautions[precautions['Disease'].str.lower() == dis][
        ['Precaution_1','Precaution_2','Precaution_3','Precaution_4']
    ].values.flatten().tolist() or []

    med = medications[medications['Disease'].str.lower() == dis]['Medication'].astype(str).tolist() or []
    die = diets[diets['Disease'].str.lower() == dis]['Diet'].astype(str).tolist() or []
    wrkout = workout[workout['disease'].str.lower() == dis]['workout'].astype(str).tolist() or []
...
def get_prediction(patient_symptoms):
    ...
    for symptom in patient_symptoms:
        ...
        if key in symptoms_dict:
            ...
            raise KeyError(f"Symptom not recognized: {symptom}")
